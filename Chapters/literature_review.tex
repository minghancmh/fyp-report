...
\subsection{Ansible}
Ansible is an open-source automation tool used for configuration management, application deployment, and task automation. Unlike Terraform and CloudFormation, which are declarative, Ansible uses an imperative approach to define the desired state of the infrastructure. The greatest upside of Ansible is that it is agentless, meaning it does not require any software to be installed on the target machines \cite{red-hat-no-date}.

\subsection{Comparison between the different frameworks}

Table \ref{tab:comparison} shows the comparison between the different IaC frameworks discussed above \cite{spitzenberger-2024} \cite{ninawe-2025} \cite{dinu-2024}.
\begin{table}[h]
    \centering
    \small % Reduce font size to fit the table
    \renewcommand{\arraystretch}{1.3}
    \begin{adjustbox}{max width=\textwidth} % Ensures the table stays within page margins
    \begin{tabular}{p{3cm} p{3cm} p{3cm} p{3cm} p{3cm}}
        \toprule
        \textbf{Framework} & \textbf{Terraform} & \textbf{AWS CloudFormation} & \textbf{OpenTofu} & \textbf{Ansible} \\
        \midrule
        \textbf{Type} & Declarative & Declarative & Declarative & Imperative + Declarative \\
        \textbf{Primary Use} & Multi-cloud IaC & AWS IaC & Multi-cloud IaC & Configuration Mgmt \\
        \textbf{Vendor Lock-in} & No & Yes & No & No \\
        \textbf{Multi-Cloud} & Yes & No & Yes & Yes \\
        \textbf{State Mgmt} & Yes & Yes & Yes & No \\
        \textbf{Execution Model} & Plan, Apply, Destroy & Stack-based Updates & Plan, Apply, Destroy & Task Execution \\
        \textbf{Language} & HCL & JSON/YAML & HCL & YAML \\
        \textbf{Extensibility} & Providers & AWS-native & Providers & Modules, Plugins \\
        \textbf{Speed} & Fast & Slower & Fast & Fast \\
        \textbf{Rollback} & Manual& Auto Rollback & Manual & Manual \\
        \textbf{Community} & Strong & AWS-focused & Growing & Very Strong \\
        \textbf{Best Use Case} & Multi-cloud infra & AWS infra automation & Open-source Terraform & Config Mgmt, Automation \\
        \bottomrule
    \end{tabular}
    \end{adjustbox}
    \caption{Comparison of Terraform, AWS CloudFormation, OpenTofu, and Ansible}
...
IaC orchestration is used to enforce good programming practices, improving code readability and quality. This provides additional structure, automation, and improves maintainability for complex infrastructure \cite{spacelift-2025}. These tools provide a significant advantage over pure Terraform by improving code readability, promoting consistency across projects, and enabling faster collaboration and development in multi-environment setups. 

Several IaC orchestration tools have also been built on top of terraform, such as terragrunt and terraspace \cite{dinu-2025B}. While an IaC orchestration tool will not be used in the implementation due to the main aim of keeping \texttt{cloudkube} portable and easily distributable, it is important to note how these tools fit into the whole cloud ecosystem.

\subsection{Terragrunt}
Terragrunt is a thin wrapper that is written on top of terraform, that provides some tooling and opinions \cite{nguyen-2020}. Terragrunt also provides a way to keep your code DRY (Don't Repeat Yourself) by using the generate helper method. Terragrunt also helps in the automated creation of backends, which can be a chore if using raw terraform. Lastly, terragrunt allows for the injection of CLI hooks, allowing the utility to execute custom actions before or after terraform commands.
...
\begin{enumerate}
    \vspace{-1em}
        \item \textbf{Self-healing: } Kubernetes restarts, replaces, or kills containers that are unhealthy, and doesn’t advertise these containers to clients until they are ready to serve \cite{quip-2021}.
        \item \textbf{Storage Orchestration:} Kubernetes allows the automatic mounting of a storage system of the developers choice \cite{quip-2021}.
        \item \textbf{Horizontal Scaling: } Kubernetes can scale up or down an application with a simple command, UI, or automatically based on user-defined parameters \cite{quip-2021}.
        \item \textbf{Service Discovery and Load Balancing: } Kubernetes can expose a container using the DNS name of the container, or using its own IP address. Kubernetes can also load balance and distribute network traffic to ensure deployment stability \cite{mannambeth-2024}. 
\end{enumerate}
...
\begin{enumerate}
    \vspace{-1em}
        ...
        The Kubernetes master node, also known as the control plane, is the controller of the whole system. This is where all operational decisions are made, including scheduling, balancing, and responding to events \cite{dinu-2025C}.  
        \begin{enumerate}
            ...
            \item \textit{etcd:} The etcd is a highly available, distributed key-value store that is use by Kubernetes to store cluster configuration data and state information. It keeps track of all cluster components such as nodes, pods, deployments and network configurations \cite{ibm-2025}. 
        \end{enumerate}
        \item \textbf{Worker Nodes}

        The Kubernetes worker node, is a worker machine in the Kubernetes cluster. There can be multiple worker nodes in a Kubernetes cluster. Each node is responsible for running pods, and is managed by the master node. These worker nodes are the workhorses of the Kubernetes cluster, and all application execution logic is carried out in these worker nodes \cite{khisty-2025}. 
        \begin{enumerate}
